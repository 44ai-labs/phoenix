...
* **PHOENIX\_ACTIVATE\_FAST\_FULL\_TEXT\_SEARCH\_PSQL:** Opt-in flag to enable accelerated full-text search on span `input.value` and `output.value` fields when using PostgreSQL. When set to `true`, Phoenix will create [pg\_trgm](https://www.postgresql.org/docs/current/pgtrgm.html) GIN indexes on those fields during database migration, and will use `LIKE`-based queries that can leverage those indexes instead of `strpos()`. This reduces substring search time from tens of seconds to milliseconds on large installations. Disabled by default because the GIN indexes consume significant disk space — typically 15–30% of the total size of the indexed text, which can be several hundred megabytes to over a gigabyte for installations with long LLM responses. Only applies to PostgreSQL; has no effect on SQLite.
